...
/* script.js
function updateValues(netWorthChange, liquidityChange, stressChange) {
    const netWorth = document.getElementById('netWorth');
    const liquidity = document.getElementById('liquidity');
    const stressLevel = document.getElementById('stressLevel');

    netWorth.value = Math.max(0, Math.min(100, netWorth.value + netWorthChange));
    liquidity.value = Math.max(0, Math.min(100, liquidity.value + liquidityChange));
    stressLevel.value = Math.max(0, Math.min(100, stressLevel.value + stressChange));
}
